    ...
    *,
    ...
    if not skills_by_category:
        ...
        if include_catalog:
            ...
            result = (
                "## Skills / Resolver\n"
                "Do not preload the full skill catalog into routine reasoning. Skills are resolved "
                "per turn from RESOLVER.md and SKILL.md frontmatter; load matched skills with "
                "skill_view(name) before acting. If no resolver hint is present but the task is "
                "clearly specialized, use skills_list/skill_view selectively. If a loaded skill is "
                "wrong or stale, patch it with skill_manage. Save reusable non-trivial workflows "
                "as skills after completion.\n"
                "Resolver source: `~/.hermes/skills/RESOLVER.md` plus profile-specific "
                "`skills/RESOLVER.md` where present. The full skill catalog is available through "
                "skills_list, but is intentionally not injected into the startup prompt."
            )
