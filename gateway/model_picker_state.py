...
class _PickerEntry:
    ...
    step: Literal["provider", "model"] = "model"
    """Current step in the two-step picker flow: 'provider' or 'model'."""
    selected_provider: Optional[str] = None
    """Provider selected in step 1 (when step='provider'). Used to filter models in step 2."""
...
class ModelPickerState:
    ...
    def mark_page(
        self, platform: str, chat_id: str, session_key: str, page: int
    ) -> bool:
        """Update the current_page of a pending picker.

        Returns True if the entry existed and was updated, False if no
        entry was found for this triple.  Does not check expiry — callers
        that care about TTL should call :meth:`lookup` first.
        """
        key: _PickerKey = (platform, chat_id, session_key)
        entry = self._store.get(key)
        if entry is None:
