...
const asStatus = (v: unknown): SubagentStatus => {
  ...
  // The delegation backend emits these terminal spellings even though Desktop
  // intentionally exposes a smaller canonical status set. Never let a timeout,
  // error, or cancellation fall through to `running`: that strands the parked
  // background notice forever after the child has already stopped.
